...
print('cross evaluation:', np.mean(cross_val_score(fit3, x, y, cv = 10)))
...
xtrain, xtest, ytrain, ytest = train_test_split(x, y, test_size = 0.25, random_state = 1)
knn_scv = KNeighborsClassifier(n_neighbors= 5)
...
print('cross evaluation:', np.mean(cross_val_score(fit8, xsa, ysa, cv = 10)))
# linsvc = LinearSVC()...
...
plot_classifiers(x.values, y.values, [knn_scv, fit8])
# %%
